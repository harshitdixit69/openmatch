-- Migration Part 2: Soft Pass State — column, RPCs, and match_profiles update.
-- Depends on migration 500 having added 'passed' to match_status_type.

-- 1. Add passed_at timestamp column to matches table
alter table public.matches
  add column if not exists passed_at timestamp with time zone;

-- 2. soft_pass_profile() — called on every normal left-swipe.
--    Creates/updates a 'passed' status with passed_at = now() (30-day TTL).
--    Safety: will NOT overwrite 'connected' or 'pending' status.
create or replace function public.soft_pass_profile(p_candidate_id uuid)
returns void
language plpgsql
security definer
...
declare
  ...
  v_user_1_id uuid;
  v_user_2_id uuid;
  v_existing_status public.match_status_type;
begin
  ...
  -- Check existing status: do not downgrade an active relationship
  select status into v_existing_status
  from public.matches
  where user_1_id = v_user_1_id and user_2_id = v_user_2_id;

  if v_existing_status in ('connected', 'pending') then
    -- Do not overwrite an accepted or pending connection with a soft pass
    return;
  end if;

  insert into public.matches (user_1_id, user_2_id, status, is_unlocked, passed_at)
  values (v_user_1_id, v_user_2_id, 'passed', false, now())
...
-- 3. hard_reject_profile() — called on explicit Block (permanent, never reintroduced)
create or replace function public.hard_reject_profile(p_candidate_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into public.matches (user_1_id, user_2_id, status, is_unlocked, passed_at)
  values (v_user_1_id, v_user_2_id, 'rejected', false, null)
  on conflict (user_1_id, user_2_id)
  do update set
    status      = 'rejected',
    passed_at   = null,
...
-- 4. Update clear_rejected_profiles() to also wipe 'passed' rows.
--    The "Reset feed" button should clear both soft-passed and hard-rejected.
...
begin
  ...
  where status in ('rejected', 'passed')
    and (user_1_id = v_viewer_id or user_2_id = v_viewer_id);
end;
$$;

-- 5. Update match_profiles() to enforce the 30-day soft-pass cooldown server-side.
--    - Always exclude: connected, pending, rejected (permanent)
--    - Exclude 'passed' ONLY IF passed_at is within the last 30 days
--    - After 30 days, soft-passed profiles automatically reappear in the feed
...
as $$
  ...
  where c.onboarding_completed_at is not null
    ...
    -- Exclusion logic:
    --   connected / pending / rejected → always excluded (permanent)
    --   passed → excluded only within the 30-day cooldown; after that, reintroduced
    and not exists (
...
grant execute on function public.soft_pass_profile(uuid) to authenticated;
grant execute on function public.hard_reject_profile(uuid) to authenticated;
