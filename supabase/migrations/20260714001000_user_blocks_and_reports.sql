-- Migration: User blocks and reports table + match_profiles filtering
...
-- Redefine match_profiles to filter out blocked users
...
as $$
  ...
  where c.id <> coalesce(p_viewer_id, auth.uid())
    ...
    and not exists (
      ...
      where (ub.blocker_id = coalesce(p_viewer_id, auth.uid()) and ub.blocked_id = c.id)
         or (ub.blocker_id = c.id and ub.blocked_id = coalesce(p_viewer_id, auth.uid()))
    )
