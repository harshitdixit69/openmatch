-- Migration: Fix match_profiles to accept explicit viewer ID
-- Replaces auth.uid() with coalesce(p_viewer_id, auth.uid()) so the caller
-- can pass the user ID directly. This prevents silent empty results when
