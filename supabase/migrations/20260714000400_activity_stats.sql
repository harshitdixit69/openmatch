-- Migration: get_activity_stats RPC
-- Aggregates activity data from existing tables into a single dashboard call.

create or replace function public.get_activity_stats(p_user_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  from public.interest_requests
  where sender_id = p_user_id and status = 'ghosted';

  -- Messages sent / received
  select
    count(*) filter (where sender_id = p_user_id)::integer,
    count(*) filter (where sender_id <> p_user_id)::integer
  into v_messages_sent, v_messages_received
  from public.messages m
  join public.matches mt on mt.id = m.match_id
  where mt.user_1_id = p_user_id or mt.user_2_id = p_user_id;
