-- Fix: Candidates with null religion/diet/marital_status should pass through filters.
-- Previously, null candidate fields fell through to a text LIKE check on the preferences
-- column. Since most users leave preferences blank, this silently excluded them.
-- Rule: null = "not filled in yet" = treat as wildcard, not a mismatch.

drop function if exists public.match_profiles(
  integer, uuid, integer, integer, integer, integer, text, text[], text, text, text, text, text, integer, integer
);
...
as $$
  ...
  where c.onboarding_completed_at is not null
    ...
    -- Religion: null on candidate means they haven't filled it in yet → treat as wildcard (pass through)
    and (
    ...
    -- Diet: null on candidate means they haven't filled it in yet → treat as wildcard
    and (
    ...
    -- Marital status: null on candidate means they haven't filled it in yet → treat as wildcard
    and (
