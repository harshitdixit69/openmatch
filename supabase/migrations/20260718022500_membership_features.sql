-- Migration: Add schema and RPCs for Super Interest and Spotlights

-- 1. Alter interest_requests to add is_super column
alter table public.interest_requests
  add column if not exists is_super boolean default false not null;

-- 2. Alter profiles to add spotlight_active_until column
alter table public.profiles
  add column if not exists spotlight_active_until timestamp with time zone;

create index if not exists profiles_spotlight_active_until_idx
  on public.profiles (spotlight_active_until desc nulls last);

-- 3. Create RPC to activate spotlight
create or replace function public.activate_spotlight()
returns json
language plpgsql
security definer
as $$
...
begin
  ...
  -- Fetch spotlights count
  select spotlights_remaining
  into v_spotlights
  from public.profiles
  where id = v_uid;

  if v_spotlights <= 0 or v_spotlights is null then
    raise exception 'No spotlights remaining';
  end if;

  -- Set active until 30 minutes from now
  v_expiry := now() + interval '30 minutes';

  update public.profiles
  set 
    spotlights_remaining = spotlights_remaining - 1,
    spotlight_active_until = v_expiry
  where id = v_uid;

  return json_build_object(
    'success', true, 
    'spotlight_active_until', v_expiry,
    'spotlights_remaining', v_spotlights - 1
  );
end;
$$;

grant execute on function public.activate_spotlight() to authenticated;

-- 4. Recreate match_profiles feed generator function to sort spotlights at the top
