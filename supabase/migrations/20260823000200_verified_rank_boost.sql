-- Migration: give the verified badge a functional payoff
--
-- Until now `verification_status` was returned by match_profiles but never used
-- for ordering, so verification cost the user a government ID and a live selfie
-- and bought them nothing but a checkmark other people see. Nobody rationally
-- opts into that.
--
-- This applies a modest additive boost to the similarity score rather than
-- sorting on `verified` ahead of similarity. Sorting on the flag first would
-- front-load the feed with verified-but-irrelevant profiles and wreck match
-- quality; a +0.03 nudge only reorders profiles that were already close.
--
-- Body is otherwise carried over verbatim from
-- 20260823000100_discovery_and_safety_settings.sql.
...
AS $$
  ...
  -- Active spotlights first, then similarity with a small verified-profile
  -- boost, then recency.
  order by
    (c.spotlight_active_until is not null and c.spotlight_active_until > now()) desc,
    (
      case
        when viewer.embedding is not null and c.embedding is not null
          then 1 - (c.embedding <=> viewer.embedding)
      ...
      + case when c.verification_status = 'verified' then 0.03 else 0 end
    ) desc,
    c.created_at desc
  limit result_limit
  offset p_offset;
