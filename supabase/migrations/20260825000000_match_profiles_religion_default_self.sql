-- Migration: Religion-aware matching
--
-- Product rule: by default a user should only see matches of their OWN religion
-- (e.g. a Hindu sees only Hindu profiles). Users can override this from
-- "Edit Preferences" by choosing a different religion, or "Any" to see all faiths.
--
-- Two changes vs. the previous match_profiles():
--   1. eff_religion now falls back to the VIEWER'S OWN religion (v.religion) when no
--      explicit param and no saved pref_religion is present.
--   2. The religion filter is now STRICT: candidates with a null/blank religion are
--      hidden whenever a specific religion is in effect (only "Any" or a viewer with
--      no religion at all shows everyone).
--
-- Also backfills existing rows: any auto-defaulted pref_religion = 'Any' is reset to
-- NULL so the "own religion" default takes effect for current users. Users who truly
-- want everyone can re-select "Any" in Edit Preferences (which persists as 'Any').

-- Reset auto-default 'Any' preferences so the own-religion default applies.
update public.profiles
set pref_religion = null
where pref_religion is not null
  and lower(pref_religion) = 'any';
...
AS $$
  with viewer as (
    select
      ...
      -- Religion default: explicit param -> saved preference -> the viewer's OWN religion.
      coalesce(
        nullif(p_religion, ''),
        nullif(v.pref_religion, ''),
        nullif(v.religion, '')
      ) as eff_religion,
  ...
  where c.onboarding_completed_at is not null
    ...
    -- Religion check (STRICT): when a religion is in effect, only same-religion
    -- candidates match. Blank-religion candidates are hidden. "Any" or a viewer with
    -- no religion at all disables the filter.
    and (
      viewer.eff_religion is null
      or lower(viewer.eff_religion) = 'any'
      or (c.religion is not null and lower(c.religion) = lower(viewer.eff_religion))
    )
    -- Diet check
    and (
