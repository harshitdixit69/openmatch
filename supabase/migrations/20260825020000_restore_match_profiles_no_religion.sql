-- Migration: Restore pre-religion match_profiles behavior
--
-- The religion-aware matching change (default to viewer's own religion + strict
-- same-religion filtering) was reverted at the product level. It had already been
-- pushed to the dev database, which left users like `harshit` seeing only a single
-- same-religion candidate. This migration recreates match_profiles exactly as it was
-- before the religion change (lenient religion clause: only filters when a religion
-- preference is explicitly set, and never hides candidates with a null religion).
--
-- This is a straight copy of 20260718023500_add_subscription_tier_to_match_profiles.sql.
